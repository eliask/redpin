/**
 * Filename: RedpinStandaloneServer.java (in org.repin.server.standalone) This
...
public class RedpinStandaloneServer implements Runnable {
    ...
    @Override
    public void run() {
        log.log(Level.INFO, "Started server at {0}:{1}", new Object[]{serverSocket.getInetAddress().getHostName(), serverSocket.getLocalPort()});
        running = true;
        try {
