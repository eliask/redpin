/**
 * Filename: ConnectionHandler.java (in org.redpin.server.standalone.net) This
...
public class ConnectionHandler implements Runnable {
    ...
    public void run() {
        ...
        log.log(Level.FINE, "Connection Handler {0}: Started Connection {1}:{2}", new Object[]{Thread.currentThread().getId(), socket.getInetAddress().getHostName(), socket.getPort()});
        try {
        ...
        log.log(Level.FINE, "Connection Handler {0}: Closed Connection {1}:{2}", new Object[]{Thread.currentThread().getId(), socket.getInetAddress().getHostName(), socket.getPort()});
